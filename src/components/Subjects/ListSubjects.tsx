...
import { getSubjects, deleteSubject } from "../../services/subjectsService";
...
const ListSubjects = () => {
  ...
  const handleDelete = async (id: string) => {
    Swal.fire({
      ...
      text: "¿Está seguro de querer eliminar esta asignatura?",
      icon: "warning",
      ...
      <div className="flex flex-col gap-9">
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark flex justify-between items-center">
            <h3 className="font-medium text-black dark:text-white">Asignaturas</h3>
            <a href="/asignaturas/crear" className="bg-primary text-white px-4 py-2 rounded-md text-sm">
              + Nueva asignatura
            </a>
          </div>
                  ...
                  {data.map((item) => (
                    <tr key={item.id} className="odd:bg-white even:bg-gray-50 border-b border-gray-200">
                      ...
                      <td className="px-6 py-4 space-x-2 flex">
                        <button onClick={() => navigate(`/asignaturas/editar/${item.id}`)} className="text-yellow-600">
                          <Edit size={20} />
                        </button>
